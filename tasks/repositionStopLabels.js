...
const index = new KDBush(stopsArr, (p) => p.lng, (p) => p.lat);
...
const stopPairs = stopsArr
  .map(stop => {
    const nearest = geokdbush.around(index, stop.lng, stop.lat, 1, maxDistance/1000, (s) => s.no !== stop.no);
    if (nearest && nearest.length){
      const stop2 = nearest[0];
      const distance = ruler.distance([stop.lng, stop.lat], [stop2.lng, stop2.lat]);
      const bearing = ruler.bearing([stop.lng, stop.lat], [stop2.lng, stop2.lat]);
  ...
  .filter(pair => {
    ...
    if (processedStops.includes(stop) || processedStops.includes(stop2)) return false;
    // Remove pairs with too little bearing difference
    if ((bearing < bearingSnap && bearing > -bearingSnap) || bearing > 180-bearingSnap || bearing < bearingSnap-180) return false;
